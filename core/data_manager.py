...
class DataManager:
    ...
    def get_income_by_entity(self, entity_id: int, year: int) -> List[Dict]:
        """获取经营主体年度收入数据"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
    ...
    def save_tax_record(self, entity_id: int, year: int, quarter: int,
        ...
        cursor.execute(
            """
            INSERT INTO tax_records (id, entity_id, year, quarter, tax_type,
            taxable_income, tax_amount, tax_rate, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (rec_id, entity_id, year, quarter, tax_type, taxable_income, tax_amount, tax_rate, notes),
        )
        conn.commit()
        conn.close()

    def get_tax_records(self, entity_id: int, year: int) -> List[Dict]:
        """获取税款记录"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
